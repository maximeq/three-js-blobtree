...
class SlidingMarchingCubes {
    ...
    _isMinCurvatureTriangulation(v1, v2, v3, v4)
    {
        ...
        //check min curvature
        let p1 = new THREE.Vector3(this.geometry.position[v1*3]
            ,this.geometry.position[v1*3+ 1] 
            ,this.geometry.position[v1*3 + 2]);
        let p2 = new THREE.Vector3(this.geometry.position[v2*3]
            ,this.geometry.position[v2*3+ 1] 
            ,this.geometry.position[v2*3+ 2]);
        let p3 = new THREE.Vector3(this.geometry.position[v3*3]
            ,this.geometry.position[v3*3+ 1] 
            ,this.geometry.position[v3*3+ 2]);
        let p4 = new THREE.Vector3(this.geometry.position[v4*3]
            ,this.geometry.position[v4*3+ 1] 
            ,this.geometry.position[v4*3+ 2]);

        //Edges from v2
        let pp_2_1 = new THREE.Vector3();
        pp_2_1.subVectors(p1,p2);

        let pp_2_3 = new THREE.Vector3();
        pp_2_3.subVectors(p3,p2);

        let pp_2_4 = new THREE.Vector3();
        pp_2_4.subVectors(p4,p2);

        //Edges from v4
        let pp_4_1 = new THREE.Vector3();
        pp_4_1.subVectors(p1,p4);

        let pp_4_3 = new THREE.Vector3();
